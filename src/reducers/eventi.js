...
REQUEST_EVENTO, RECEIVE_NEW_EVENTO, RECEIVE_CACHED_EVENTO, DELETED_EVENTO, SAVED_EVENTO } from '../actions/eventi';
...
function eventi(state = initialState, action) {
  switch (action.type) {
    ...
    case RECEIVE_NEW_EVENTO:
      const itemsR = cloneDeep(state[state.selezionati].items);
      itemsR[action.id] = action.evento[action.id];
      return Object.assign({}, state, { isLoading: false, [state.selezionati]: { items: itemsR }, dettaglio: action.id  });
    case DELETED_EVENTO:
